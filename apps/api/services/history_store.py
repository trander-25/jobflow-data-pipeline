...
class MongoChatHistoryStore:
    """Persist and retrieve chatbot conversation history from MongoDB."""

    def __init__(self, settings: Settings, client=None, collection=None):
        """Create a MongoDB collection handle, or use an injected collection for tests."""
        self.client = client
    ...
    def healthcheck(self) -> None:
        """Verify MongoDB is reachable when a real client is configured."""
        if self.client is None:
            return
        self.client.admin.command("ping")

    def add_message(self, user_id: str, role: str, message: str) -> None:
        """Append one chat message to a user's conversation history."""
    ...
    def clear_user(self, user_id: str) -> int:
        """Delete all stored chat messages for one user."""
